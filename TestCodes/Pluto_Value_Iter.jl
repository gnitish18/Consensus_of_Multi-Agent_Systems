...
# ╔═╡ 7644dfa0-4f3c-11ec-2443-113ce7fccd35
using POMDPs, QuickPOMDPs, POMDPPolicies, POMDPModelTools

# ╔═╡ a8268f1e-44e8-4527-8a3a-6a2fb16f6e96
using Parameters, Random

# ╔═╡ 0d3f2c8e-389d-4604-9627-0dd679bf6660
using Plots; default(fontfamily="Computer Modern", framestyle=:box) # LaTex-style

# ╔═╡ d5b4bee6-642a-4fcf-8da1-83ea97e68d3e
@with_kw struct ConsensusProblem
	
	# Rewards
	r_statechange::Real = -100
	r_interact::Real = -500
	r_concensus::Real = 1000
	r_wisdom::Real = 500
	r_final::Real = 10000
...
# ╔═╡ 51258e4a-6773-4a39-a2a4-a6466ca4087a
params = ConsensusProblem();

# ╔═╡ 81229d69-e537-4798-91a4-92fd17411deb
begin
	n_states = 3
	n_actions = 2
	n_agents = 3
	@enum State CIRCLEₛ SQUAREₛ TRIANGLEₛ
	@enum Action IGNOREₐ INTERACTₐ 
	@enum Agent A B C
	All_States = zeros(n_states^n_agents)
end

# ╔═╡ 5382e55e-5f93-4e3c-ace5-af9cf3914b87
function Statespace()
	All_States = [(i-1)*9 + (j-1)*3 + (k-1) for i in 1: n_states for j in 1: n_states for k in 1: n_states]
	return All_States
end

# ╔═╡ fe4e1c27-beb9-4861-8897-feaf2eca2755
function decode_States(index)
	i = All_States[index] % n_states
	j = (All_States[index] / n_states) % n_states
	k = (All_States[index] / n_states^2) % n_states
	return [All_States[index], i, j, k]
end

# ╔═╡ 31d203bc-718a-4053-83e9-871df65b9ebc
begin
	Statespace()
	decode_States(26)
end

# ╔═╡ d1da9ca5-e219-4cd1-a89b-dc9609e9a41d
𝒮 = [CIRCLEₛ, SQUAREₛ, TRIANGLEₛ]

# ╔═╡ 67584530-2976-4c30-9564-aaa09bc605bf
𝒜 = [IGNOREₐ, INTERACTₐ]

# ╔═╡ 67df5edd-fdf3-42a1-97c6-5bfed5bace7c
𝒜𝒢 = [A, B, C]

# ╔═╡ 3ec738e4-123b-4fa3-bfb3-ec8bc5d93134
function T(s::State, a::Action, ag::Agent)
	p_A[1] = params.p_A_a0
	p_A[2] = params.p_A_a1
	p_B[1] = params.p_B_a0
	p_B[2] = params.p_B_a1	
	...
	for i in 1:3
		if a == IGNOREₐ
			if s == CIRCLEₛ
				return SparseCat([CIRCLEₛ, SQUAREₛ, TRIANGLEₛ], [p_A[1], (1-p_A[1])/2, (1-p_A[1])/2])
			elseif s == SQUAREₛ
				return SparseCat([CIRCLEₛ, SQUAREₛ, TRIANGLEₛ], [(1-p_A[1])/2, p_A[1], (1-p_A[1])/2])
			elseif s == TRIANGLEₛ
				return SparseCat([CIRCLEₛ, SQUAREₛ, TRIANGLEₛ], [(1-p_A[1])/2, (1-p_A[1])/2, p_A[1]])
			end
		elseif a == INTERACTₐ
			if s == CIRCLEₛ
				return SparseCat([CIRCLEₛ, SQUAREₛ, TRIANGLEₛ], [p_A[2], (1-p_A[2])/2, (1-p_A[2])/2])
			elseif s == SQUAREₛ
				return SparseCat([CIRCLEₛ, SQUAREₛ, TRIANGLEₛ], [(1-p_A[2])/2, p_A[2], (1-p_A[2])/2])
			elseif s == TRIANGLEₛ
				return SparseCat([CIRCLEₛ, SQUAREₛ, TRIANGLEₛ], [(1-p_A[2])/2, (1-p_A[2])/2, p_A[2]])
			end
		end
	end
end

# ╔═╡ 63e80b06-370f-453f-baf2-fe3f5dd48b3f
function R(s::State, a::Action, s̃ ::State)
	params.r_statechange = -100
	params.r_interact = -500
	params.r_concensus = 1000
	params.r_wisdom = 500
	params.r_final = 10000
end

# ╔═╡ 4652b2b5-d413-4bca-be20-8bce62d01bf5
SparseCat([CIRCLEₛ, SQUAREₛ, TRIANGLEₛ], [0.7, 1, 0.2])
...
git-tree-sha1 = "45eb33be463de0e793b7dcd0e18fdc73cf7ceed6"
